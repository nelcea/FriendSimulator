...
extension BLEManager: CBPeripheralManagerDelegate {
    ...
    func peripheralManagerDidUpdateState(_ peripheral: CBPeripheralManager) {
        ...
        if peripheral.state == .poweredOn {
            ...
            /*
            let batteryLevelCharacteristic = CBMutableCharacteristic(type: BatteryService.batteryLevelCharacteristicUUID, properties: .read, value: nil, permissions: .readable)
            let batteryService = CBMutableService(type: BatteryService.serviceUUID, primary:true);
            batteryService.characteristics = [batteryLevelCharacteristic]

            manager.add(batteryService)
            // TODO: seems Apple forbids to publish standard define services
            */
